...
#         enclose_area = cw * ch + eps
#
#         enclosure_penalty = (enclose_area - union) / enclose_area
#
#         # ---- 2️⃣ 中心漂移（小目标稳定）
#         center_dist = (
...
#         ) / 4
#
#         enclose_diag = cw.pow(2) + ch.pow(2) + eps
#         center_penalty = center_dist / enclose_diag
#
#         # ---- 3️⃣ 形态自适应约束（火焰/烟雾非刚性）
#         shape_penalty = (
#             torch.atan(w2 / (h2 + eps)) -
#             torch.atan(w1 / (h1 + eps))
#         ).pow(2)
#
#         # ---- 权重（可根据数据集微调）
#         lambda_c = 0.6   # 中心稳定（小火点）
#         lambda_s = 0.3   # 形态对齐（烟雾）
#         lambda_e = 0.2   # 背景抑制
#
#         return iou - (
#             lambda_c * center_penalty +
#             lambda_s * shape_penalty +
#             lambda_e * enclosure_penalty
#         )
#
#     # -------------------------
#     # original behavior
...
# def bbox_iou(box1, box2, xywh=True, GIoU=False, DIoU=False, CIoU=False, SDIoU=False, eps=1e-7,delta=0.5, d=0.5):
#     """
...
#                 if SDIoU:
#                     beta = (w2 * h2 * delta) / 81
#                     beta = torch.where(beta > delta, torch.tensor(delta).cuda(), beta)
